...
class Photonfocus:
    def __init__(self):
        ...
        self.pixelChar = {}
        self.calibrationFile = os.path.join(
    ...
    def GetCalibrationData(self,calibrationFile=None, display= False):
        ...
        if display:
            ...
            f.suptitle('Spectral deconvolution matrix')
            plt.show()
    ...
    def imageDataToSpectralCube(self,imageData,darkImage=None):
        ...
        # renaming and substracting darkImage
        image = imageData  - self.darkImage

        # works only for camera with 25 pixel per super-pixel
        #select only full superpixels
        im2D = np.squeeze(image[self.chipOffset[0]:(image.shape[0]-self.chipOffset[0])//5*5+self.chipOffset[0], \
                                self.chipOffset[1]:(image.shape[1]-self.chipOffset[1])//5*5+self.chipOffset[1]])
